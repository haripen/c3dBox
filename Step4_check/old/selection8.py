...
class SelectionManager:
    def __init__(self, ax: Axes, line_to_item: Dict[Line2D, SelectionItem], mode: str = "select",
                 on_result: Optional[Callable[[List[SelectionItem], str, Bbox], None]] = None) -> None:
        self.ax = ax
        ...
        self._rs = RectangleSelector(
            self.ax, onselect=self._on_select, useblit=True, button=[1],
            spancoords="data", interactive=False, drag_from_anywhere=True, minspanx=0.0, minspany=0.0
        )
    ...
    def _on_select(self, eclick, erelease) -> None:
        ...
        for ln, item in self._line_to_item.items():
            if _line_intersects_rect(ln, bbox):
                hits.append(item)
        print(f"[rect] hits={len(hits)} mode={self.mode}")
        if self.on_result is not None:
            try:
                self.on_result(hits, self.mode, bbox)
            except Exception as e:
                print(f"[rect] callback error: {e!r}")
