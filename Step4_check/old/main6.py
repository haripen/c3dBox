...
LEFT_COLOR = "#d62728"; RIGHT_COLOR = "#1f77b4"
...
class PlotCell(QtWidgets.QWidget):
    ...
    def __init__(self, parent, title: str, coord_index: Optional[int], combo_width_chars: int = 7,
                 key_resolver: Optional[Callable[[str, str], str]] = None):
        super().__init__(parent)
        self.title = title; self.coord_index = coord_index; self.current_display_key = ""
        ...
        top = QtWidgets.QHBoxLayout(); lab = QtWidgets.QLabel(self.title); lab.setMinimumWidth(100)
        top.addWidget(lab); top.addStretch(1); top.addWidget(self.combo)
    ...
    def _on_param_changed(self, disp: str):
        self.current_display_key = disp.strip(); self.parent().parent().parent().parent().redraw_all()
    def clear(self):
        self.ax.cla()
        if self.selector is not None: self.selector.disconnect(); self.selector=None
        self.canvas.draw_idle()
    def plot(self, files: List[LoadedFile], display_key: str, show_left: bool, show_right: bool, mode_text: str,
        ...
        disp = display_key
        if self.coord_index is not None and "[" not in disp and not disp.endswith("*"):
            disp = f"{disp} [{'xyz'[self.coord_index]}]"
        self.ax.set_ylabel(disp); self.ax.set_xlabel("% cycle"); self.ax.set_title(f"{self.title}   |   Mode: {mode_text}")
        self.canvas.draw_idle()
...
class PlotPage(QtWidgets.QWidget):
    ...
    def __init__(self, parent, title: str, rows: int, cols: int, coord_map: List[Optional[int]],
        ...
        super().__init__(parent)
        self.title = title; grid = QtWidgets.QGridLayout(self); grid.setContentsMargins(6,6,6,6); grid.setSpacing(6)
        self.cells: List[PlotCell] = []; k=0
        for r in range(rows):
            for c in range(cols):
                cell = PlotCell(self, f"{title} [{r+1},{c+1}]", coord_index=coord_map[k], combo_width_chars=7,
    ...
    def redraw(self, files: List[LoadedFile], show_left: bool, show_right: bool, mode_text: str,
        ...
        for cell in self.cells:
            ...
            if disp: cell.plot(files, disp, show_left, show_right, mode_text, filter_kinetic, filter_kinematic)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        ...
        for chk in (self.chk_left,self.chk_right,self.chk_kinetic,self.chk_kinematic): chk.toggled.connect(self.on_filters_changed)
        self.lbl_mode = QtWidgets.QLabel("Mode: SELECT")
        self.btn_autoscale = QtWidgets.QPushButton("↕︎ Autoscale Y (all)"); self.btn_autoscale.clicked.connect(self.autoscale_all)
        ...
        self.page1 = PlotPage(self, "Page1", 3, 6, coord_map1, key_resolver=self.to_actual_key)
        self.page2 = PlotPage(self, "Page2", 3, 6, coord_map2, key_resolver=self.to_actual_key)
    ...
    def set_root(self, root: str):
        ...
        if self.participants: self.cmb_pid.setCurrentIndex(0); self.on_pid_changed(self.participants[0])
            ...
            self.cmb_trial.clear(); self.loaded=[]; self.display_keys=[]; self._refresh_counts(); self.redraw_all()
    def on_pid_changed(self, pid: str):
        ...
        if types: self.cmb_trial.setCurrentIndex(0); self.on_trial_changed(types[0])
            ...
            self.loaded=[]; self.display_keys=[]; self._refresh_counts(); self.redraw_all()
    def on_trial_changed(self, trial: str):
        ...
        self.page1.set_options_all(self.display_keys, self.default_displays_page1); self.page2.set_options_all(self.display_keys, self.default_displays_page2)
    ...
    def on_filters_changed(self, _state: bool): self.redraw_all()
    ...
    def autoscale_all(self):
        for page in (self.page1,self.page2):
            for cell in page.cells:
                if hasattr(cell, "autoscale"): cell.autoscale()
    @QtCore.Slot(list)
    def apply_selection(self, changes: List[tuple]):
        ...
        if changed:
            print(f"[select] applied {changed} change(s)"); self._refresh_counts(); self.redraw_all()
